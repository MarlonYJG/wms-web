...
import type { Supplier, SupplierQuery } from "@@/apis/supplier/type"
import { createSupplier, deleteSupplier, getSupplierList, updateSupplier, updateSupplierStatus } from "@@/apis/supplier"
...
async function fetchList() {
  loading.value = true
  try {
    const page = await getSupplierList(queryParams)
...
function openCreate() {
  ...
  Object.assign(formData, { supplierCode: "", supplierName: "", contactPerson: "", contactPhone: "", email: "", address: "", isEnabled: true })
...
async function handleSubmit() {
  try {
    if (formType.value === "create") {
      ...
      await updateSupplier(currentId.value!, formData)
      ElMessage.success("更新成功")
    }
    dialogVisible.value = false
    fetchList()
  } catch (e) {
    // 交由 smartRequest 统一提示
  }
}

async function handleDelete(row: Supplier) {
  await ElMessageBox.confirm(`确定要删除供应商“${row.supplierName}”吗？`, "确认删除", { type: "warning" })
  await deleteSupplier(row.id)
  ElMessage.success("删除成功")
  fetchList()
}

async function handleToggle(row: Supplier) {
  const target = !row.isEnabled
  await updateSupplierStatus(row.id, target)
...
<template>
  <div class="supplier-container">
    ...
    <el-dialog v-model="dialogVisible" :title="formType === 'create' ? '新增供应商' : '编辑供应商'" width="640px">
      <el-form :model="formData" label-width="100px">
        <el-form-item label="编码">
          <el-input v-model="(formData.supplierCode as string)" maxlength="50" show-word-limit />
        </el-form-item>
        <el-form-item label="名称">
          <el-input v-model="(formData.supplierName as string)" maxlength="100" show-word-limit />
        </el-form-item>
